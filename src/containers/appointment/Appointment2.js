...
export default class Appointment extends Component {
  constructor(props){
    ...
    this.state = {
      tasks_sid:this.props.tasks_sid,
      data:{},
      finished: false,
      stepIndex: 0,
      indexFinished:2,
  ...
  render(){
    ...
    if(this.state.appointment_editing){
      ...
      // console.log(minHour);
      // <ContentCreate onTouchTap={this.handleEditingAppointment} style={{color:lightBlack}} />
      appointment_element = <div>
          <div style={{padding:10}}>
            <div><span>Appointment: </span></div>
            <div>
              <DatePicker
    ...
    if(!this.state.can_checkpoint){
      ...
      if(this.state.data.appointment_type>="2"){
        ...
        <Paper zDepth={2} >
          <div >
            <div style={{padding:10}}><span >Subject: </span>
            <span style={{color:lightBlack,marginLeft:10}}>{this.state.data.no_task+" "+this.state.data.subject_service_report}</span>
            </div>
            <Divider />
            {appointment_element}
          </div>
            ...
            <div >
              ...
              <div style={{padding:10}}>
                ...
                {this.state.data.associate_engineer.map((item2,i2)=>{
                  return <Avatar key={i2} src={item2.picture_profile} size={40} style={{margin:4}} />
                })}
                </div>
              </div>
              <Divider />
