...
class RenderCase extends Component {
  ...
  renderRow = (row, index) => {
    ...
    return (
      <ListItem key={index} onTouchTap={()=>{navigator.pushPage({component:PageTwo,key:'PageTwo',sid:row.sid,title:row.no_ticket}) }}>
        <div className='left'>
        </div>
        <div className='center'>
          ...
          <div style={{display:'flex',width:'100%'}}>
            <div style={{flex:1}}>Created {row.create_by_name}</div>
            <div style={{flex:1,textAlign:'right'}}>{row.create_datetime}</div>
          </div>
        </div>
      </ListItem>
    );
